...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\musichackspace Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\musichackspace Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\musichackspace manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\musichackspace.vst3\Contents\\x86_64-win\musichackspace.vst3",
      projectpath + "\\build-win\musichackspace_x64.exe"  
    ]

  zipname = "musichackspace-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\musichackspace-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\musichackspace-app_x64.pdb"  
  ]
